...
let rec read_remote buf = 
    Unix.sleepf 0.1;
    let rcvd = net_reader buf 0 buffer_len in write_local buf rcvd
and write_local buf len = 
    let _sent = nblk_call Unix.write lsocket_w buf 0 len in read_local buf
and read_local buf = 
    let rcvd = nblk_call Unix.read lsocket_r buf 0 buffer_len in write_remote buf rcvd
and write_remote buf len = 
    let _sent = net_writer buf 0 len in read_remote buf
